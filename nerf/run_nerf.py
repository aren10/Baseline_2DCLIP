...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False, saliency = False, clip = False):
    ...
    raw2alpha = lambda raw, dists, act_fn=torch.sigmoid: 10.*(1.-torch.exp(-act_fn(raw)*dists))
...
def train(env, flag, test_file, i_weights):
    ...
    if args.render_test: #False by default
    ...
    if args.render_only: #False. Test' number of rays > train's number of rays
        ...
        with torch.no_grad():
            if args.render_test:
                ...
                clips = clips[i_test]
                #print(clips.shape) (2, 256, 256, 768)
            ...
            if args.with_clip:
                ...
                clips_ests_normalized = torch.tensor(clips_ests)
    ...
    for i in trange(start, N_iters):
        ...
        if args.with_saliency:
            ...
            clip_est, disp, acc, extras = render(H, W, K, chunk=args.chunk, rays=batch_rays,
                                    verbose=i < 10, retraw=True,
                                    **render_kwargs_train, use_saliency= False, use_CLIP = True)
            clips_ests_normalized = clip_est
        ...
        if args.with_saliency:
            ...
            if(img_i in [0]):
                ...
                print(clips_ests_normalized[0][0:10])
            img_loss = clip_loss(clips_ests_normalized, clip_s) #torch.Size([4096, 768])
        else:
